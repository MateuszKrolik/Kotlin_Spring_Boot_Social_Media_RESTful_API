...
gcloud iam service-accounts create build-sbsa-dev-sa \
  --description="Service account for build process in Spring Boot Social API development environment" \
  --display-name="Build SBSA Dev SA" \
  --project=spring-boot-social-api

SA_EMAIL="build-sbsa-dev-sa@$GCLOUD_ID.iam.gserviceaccount.com"
...
gcloud projects get-iam-policy $GCLOUD_PROJECT_NUMBER  \
  ...
  --filter="bindings.members:build-sbsa-dev-sa@spring-boot-social-api.iam.gserviceaccount.com"
